 ...
 *
 * プラグインコマンド:
 *   namePop1 1 名前         # イベント１番に名前をセット
 *   namePop1 1 名前 -48     # イベント１番に名前をセット、上へ４８ドットずらす
 *
 *   ネームポップを消去したい場合は namePop1 1 を実行してください。
 *   名前が省略されるとネームポップが消去されます。
 * 
 * メモ欄（イベント）タグ:
 *   <namePop1:名前>         # 名前をイベントの頭上に表示
 *   <namePop1Y:12>          # ネームポップの位置を下へ１２ドットずらす
 * 
...
(function() {
  ...
  var parameters = PluginManager.parameters('TMnamePop1');
  ...
  Game_CharacterBase.prototype.setnamePop1 = function(namePop1, shiftY) {
    if (namePop1) {
      namePop1 = Window_Base.prototype.convertEscapeCharacters.call(
                new Window_Base(0, 0, 0, 0), namePop1);
    }
    this._namePop1 = namePop1;
    this._namePop1Y = shiftY;
  };
  ...
  Game_Event.prototype.setupPage = function() {
    _Game_Event_setupPage.call(this);
    this.setnamePop1(null, 0);
    if (this._pageIndex >= 0) {
      this.setnamePop1(this.loadTagParam('namePop1'), +this.loadTagParam('namePop1Y'));
    }
  };
  ...
  Game_Interpreter.prototype.pluginCommand = function(command, args) {
    ...
    if (command === 'namePop1') {
      ...
      if (character) {
        character.setnamePop1(args[1], Number(args[2] || 0));
      }
    }
    if (command === 'TMnamePop1') {
      switch (args[0]) {
      case 'set':
        var character = this.character(args[1]);
        if (character) {
          character.setnamePop1(args[2], Number(args[3] || 0));
        }
        break;
      }
    }
  ...
  Sprite_Character.prototype.updatenamePop1 = function() {
    if (this._namePop1 !== this._character._namePop1) {
      this._namePop1 = this._character._namePop1;
      if (this._namePop1) {
        if (!this._namePop1Sprite) {
          this._namePop1Sprite = new Sprite_TMnamePop1();
          this.addChild(this._namePop1Sprite);
          this._namePop1Sprite.y = this.namePop1ShiftY();
        }
        this._namePop1Sprite.refresh(this._namePop1);
      } else {
        this.removeChild(this._namePop1Sprite);
        this._namePop1Sprite = null;
      }
    }
  };

  Sprite_Character.prototype.namePop1ShiftY = function() {
    return -this.patternHeight() + this._character._namePop1Y;
  };
  ...
  function Sprite_TMnamePop1() {
    this.initialize.apply(this, arguments);
  }

  Sprite_TMnamePop1.prototype = Object.create(Sprite.prototype);
  Sprite_TMnamePop1.prototype.constructor = Sprite_TMnamePop1;

  Sprite_TMnamePop1.prototype.initialize = function() {
  ...
  Sprite_TMnamePop1.prototype.update = function() {
    Sprite.prototype.update.call(this);
    this.y = this.parent.namePop1ShiftY();
  };

  Sprite_TMnamePop1.prototype.refresh = function(text) {
    this.bitmap.clear();
  ...
  Sprite_TMnamePop1.prototype.convertEscapeCharacters = function(text) {
  ...
  Sprite_TMnamePop1.prototype.textColor = function(n) {
    var px = 96 + (n % 8) * 12 + 6;
